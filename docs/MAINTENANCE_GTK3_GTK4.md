...
| Recent workspaces | File-menu submenu refilled from `settings.recent_folders` | primary-menu submenu (`Gio.Menu`) refilled by `_rebuild_recent_menu`; entries target `win.open-recent` with the folder path |
